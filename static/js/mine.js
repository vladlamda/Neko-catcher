...
onload=function(){
  ...
  $('#skip').click(function(){nextajax(skip=1);});
  $('#next').live('click',function(){nextajax(skip=0);});
...
function restestatus(){
  coords=new Array();
  draw();
}

function nextajax(skip){
  console.log('座標is:'+coords);
  ...
  $.ajax({
    ...
    success:function(data){
      ...
      if(finished){
        ...
        $('#canvas-wrapper').empty().append('<div class="message"><div class="message">'+imgnum+'Images were</div><div class="message">Successfuly Proceseed!</div></div>');
